...
class WalkForwardConfigLike(Protocol):
    window_type: str
...
def _validate_design_config(cfg: FeatureConfigLike, data: pd.DataFrame) -> None:
...
def build_forecasting_design_matrix(
    data: pd.DataFrame,
    config: FeatureConfigLike,
    *,
    target_transform: str = "none",
) -> tuple[pd.DataFrame, list[str], str]:
    _validate_design_config(config, data)
...
def transform_target(y: pd.Series, model_cfg: ModelConfigLike) -> pd.Series:
