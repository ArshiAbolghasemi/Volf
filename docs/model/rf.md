...
1. **Tree-level**: Each tree trained on separate CPU core (n_jobs=-1)
2. **Experiment-level**: Multiple experiments run concurrently
...
- Start with HAR core features for baseline
- Add endogenous features for commodity-specific dynamics
- Include climate variables for agricultural commodities
- Add news/macro for market-driven components

### Model Configuration
- Use default 500 trees for stability
- Limit max_depth (20-30) for large feature sets to prevent overfitting
- Increase min_samples_split/leaf for noisy data
- Use expanding window for stable long-term patterns
- Use rolling window for adaptive forecasting in changing markets

### Target Mode Selection
- **Point targets**: For specific date forecasting, tactical trading
- **Mean targets**: For risk management, strategic planning, smoother predictions

### Computational Considerations
- Random Forest is computationally intensive with large feature sets
- Use caching to avoid redundant computation
- Leverage parallel processing (n_jobs=-1)
- Consider feature selection for very high-dimensional problems
