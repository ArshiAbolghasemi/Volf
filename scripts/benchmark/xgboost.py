...
def parse_args(
    *,
    ...
    parser = argparse.ArgumentParser(description="Run Wheat XGBoost benchmark on ag/v4.csv")
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Cache directory for XGBoost benchmark artifacts",
    )
    parser.add_argument(
        "--cache_overwrite",
        action="store_true",
...
def _load_config_from_json(path: str) -> WheatXGBBenchmarkConfig:
...
def main(  # noqa: C901, PLR0912
    *,
    ...
    if args.config:
        ...
        logger.info("Loaded XGBoost benchmark config from %s", args.config)
    else:
        ...
        logger.info("Using default/CLI XGBoost benchmark config")
    if args.config:
    ...
    if cfg.target_horizons and len(cfg.target_horizons) > 1:
        results_by_horizon = run_wheat_xgb_benchmark_multi_horizon(config=cfg)
    ...
    for horizon in horizons:
        ...
        logger.info("Saved horizon=%d XGBoost summary to %s", horizon, horizon_out)

    logger.info(
        "Top XGBoost results by test_r2:\n%s",
        summary[
            [
                "target_horizon",
                "model_type",
